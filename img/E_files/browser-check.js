define(function () {
  ...
  function parseUserAgent(userAgent) {
    ...
    var match =
      /(edge)\/([\w.]+)/.exec(ua) || //make sure test edge before Chrome, because "chrome" keyword is in Edge's UA
      /(chrome)[ \/]([\w.]+)/.exec(ua) ||
      /(firefox)[ \/]([\w.]+)/.exec(ua) ||
    ...
    if (match[2] === "safari") {
      ...
      compatibilityMode =
        ua.indexOf("compatible") >= 0 && match[2] !== actualMsieVersion; // IE9 reports itself has "compatible" in default mode
